...
x_training, x_testing, y_training, y_testing = train_test_split(
    x_padded, y_encoded, test_size=0.5
)

# Define the model
model = tf.keras.models.Sequential()
model.add(tf.keras.layers.Embedding(input_dim=len(tokenizer.word_index) + 1, output_dim=8, input_length=max_sequence_length))
...
# Train the model
model.fit(x_training, y_training, epochs=100, validation_data=(x_testing, y_testing))

# Evaluate the model
model.evaluate(x_testing, y_testing, verbose=2)

# Function to predict opening based on new moves
...
# Example usage
new_moves = "d4 Nf6 c4 g6 g3 d5 Bg2 Bg7 Nf3 O-O cxd5 Nxd5 O-O Nb6"
